...
def configure_ngrok():
    ngrok_token = "2pGmSCpBqqIqWANUzlPGBA85cXK_5zcv3kY7c4Ys9dTKt3MZh"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("binder9.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in binder9.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("binder9.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated binder9.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update binder9.txt on VPS: {str(e)}")
